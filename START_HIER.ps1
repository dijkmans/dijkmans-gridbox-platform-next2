...
if ($res -match 'PYTHON_SUCCESS') {
    Write-Host "[OK] Cloud koppeling gelukt!" -ForegroundColor Green
} else {
...
Write-Host '[SYNC] Bestanden synchroniseren...' -ForegroundColor Gray
if (!(Test-Path '.git')) {
    git init | Out-Null
...
Write-Host " [KLAAR] ALLES IS KLAAR VOOR DE START!" -ForegroundColor Green
Write-Host "=================================================" -ForegroundColor Cyan
...
Write-Host "[OK] Veilige SD-kaart gedetecteerd op schijf $Drive!" -ForegroundColor Green
...
if ($ImagerPath) {
    ...
    Write-Host "[INFO] We proberen de Imager via Windows te openen..." -ForegroundColor Yellow
    try {
        Start-Process "rpi-imager" -ErrorAction Stop
    } catch {
        Write-Host "[FOUT]: Imager niet gevonden. Open deze handmatig via je Start-menu." -ForegroundColor Red
    }
}
