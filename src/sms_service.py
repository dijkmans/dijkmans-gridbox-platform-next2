...
API_KEY = "yWBz6z2JM6jMrz6Unldh4t7WswcK8pVWVc1M"
...
def send_sms(phone_number, template_id, box_id, customer_name="Klant"):
    ...
    try:
        ...
        if not template_doc.exists:
            print(f"⚠️ Fout: Template '{template_id}' niet gevonden in database!")
            return False
        ...
        if response.status_code == 201:
            print(f"✅ SMS '{template_id}' naar {phone_number} verzonden.")
            return True
        else:
            print(f"⚠️ SMS fout ({response.status_code}): {response.text}")
            return False

    except Exception as e:
        print(f"❌ SMS netwerkfout: {e}")
        return False