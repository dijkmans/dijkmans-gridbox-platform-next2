...
router.post("/admin/provisioning/:id/generate-script", async (req, res) => {
  try {
    ...
    const masterImageObject = gcsStorage
      ...
      .file("master-images/Gridbox_master_v1.0.54b.img");
    ...
    const script = [
      ...
      "$ImagePath  = \"$env:USERPROFILE\\Downloads\\Gridbox_master_v1.0.54b.img\"",
      `$GcsUrl     = "${signedUrl}"`,
      "",
      ...
      "    if ($vol -and $vol.DriveLetter) {",
      "        $bootDriveLetter = $vol.DriveLetter",
      "        Write-Host \"Bootpartitie gevonden via FAT32 fallback op ${bootDriveLetter}:\" -ForegroundColor Green",
      "    }",
      "}",
      "",
      "if (-not $bootDriveLetter) {",
