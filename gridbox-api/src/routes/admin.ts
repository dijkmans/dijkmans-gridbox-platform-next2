...
router.post("/admin/provisioning/:id/generate-script", async (req, res) => {
  try {
    ...
    const cloudInitUserData = [
      ...
      "    content: |",
      "      #!/bin/bash",
      "      LOG=\"/var/log/gridbox-bootstrap.log\"",
      "      exec > >(tee -a \"$LOG\") 2>&1",
      "      echo \"[$(date -Iseconds)] Bootstrap gestart\"",
      ...
      "      RPI_CONNECT_AUTH_KEY=$(python3 -c \"import json,sys; d=json.load(open(sys.argv[1])); print(d.get('rpiConnectAuthKey',''))\" \"$BOOTSTRAP_JSON\")",
      "      if [[ ! \"$API_BASE_URL\" =~ ^https?:// ]]; then",
      "        echo \"[$(date -Iseconds)] FOUT: apiBaseUrl ongeldig formaat: $API_BASE_URL\"",
      "        exit 1",
      "      fi",
      "      API_BASE_URL=\"${API_BASE_URL%/}\"",
      "      echo \"[$(date -Iseconds)] boxId=$BOX_ID apiBaseUrl=$API_BASE_URL\"",
      "      hostnamectl set-hostname \"$BOX_ID\"",
      "      if grep -q \"127.0.1.1\" /etc/hosts; then",
      "        sed -i \"s/^127\\.0\\.1\\.1.*/127.0.1.1\\t$BOX_ID/\" /etc/hosts",
      "      else",
      "        printf \"127.0.1.1\\t%s\\n\" \"$BOX_ID\" >> /etc/hosts",
      "      fi",
      "      apt-get update -qq",
      ...
      "        python3-gpiozero libgpiod2 i2c-tools rpi-connect",
      ...
      "          -X POST \"$API_BASE_URL/device/bootstrap/claim\" \\",
      ...
      "      done",
      "      if [ \"$CLAIM_OK\" != \"1\" ]; then",
      "        echo \"WAARSCHUWING: bootstrap claim mislukt na 5 pogingen, listener zal het opnieuw proberen\"",
      "      fi",
      "      if [ ! -d \"$WORK_DIR/.git\" ]; then",
      "        GIT_TERMINAL_PROMPT=0 timeout 300 git clone --depth 1 \\",
      "          https://github.com/dijkmans/dijkmans-gridbox-platform-next2.git \\",
      "          /tmp/gridbox-clone-tmp",
      "        cp -r /tmp/gridbox-clone-tmp/. \"$WORK_DIR/\"",
      "        rm -rf /tmp/gridbox-clone-tmp",
      "      fi",
      "      if [ ! -d \"$WORK_DIR\" ] || [ ! -f \"$WORK_DIR/requirements.txt\" ]; then",
      "        echo \"FOUT: WorkingDirectory $WORK_DIR ontbreekt of onvolledig na git clone, afbreken\"",
      "        exit 1",
      "      fi",
      "      if [ -f \"$SA_SRC\" ]; then",
      "        cp \"$SA_SRC\" \"$WORK_DIR/service-account.json\"",
      "        chmod 600 \"$WORK_DIR/service-account.json\"",
      "      fi",
      ...
      "      echo \"[$(date -Iseconds)] gridbox.service actief\"",
      "      PI_UID=$(id -u pi)",
      "      sudo -u pi XDG_RUNTIME_DIR=/run/user/$PI_UID \\",
      "        systemctl --user enable rpi-connect.service || true",
      "      sudo -u pi XDG_RUNTIME_DIR=/run/user/$PI_UID \\",
      "        systemctl --user start rpi-connect.service || true",
      "      sleep 5",
      "      if [ -n \"$RPI_CONNECT_AUTH_KEY\" ]; then",
      "        echo \"[$(date -Iseconds)] rpi-connect signin met auth key\"",
      "        sudo -u pi XDG_RUNTIME_DIR=/run/user/$PI_UID \\",
      "          rpi-connect signin --auth-key \"$RPI_CONNECT_AUTH_KEY\" || \\",
      "          echo \"[$(date -Iseconds)] WAARSCHUWING: rpi-connect signin mislukt\"",
      "      else",
      "        echo \"[$(date -Iseconds)] Geen rpiConnectAuthKey aanwezig in box_bootstrap.json\"",
      "      fi",
      "      echo \"[$(date -Iseconds)] Bootstrap volledig voltooid voor $BOX_ID\"",
    ...
    const script = [
      ...
      "",
      "# Schrijfbeveiliging automatisch verwijderen en schijf leegmaken",
      "Write-Host \"Schrijfbeveiliging verwijderen via diskpart...\" -ForegroundColor Gray",
      "$dpPre = \"select disk $diskNumber`r`nattributes disk clear readonly`r`nclean`r`nexit\"",
      "$dpPreTmp = \"$env:TEMP\\gridbox-diskpart-pre.txt\"",
      "[System.IO.File]::WriteAllText($dpPreTmp, $dpPre)",
      "$dpPreOut = & diskpart /s $dpPreTmp 2>&1",
      "Remove-Item $dpPreTmp -ErrorAction SilentlyContinue",
      ...
      "}",
      "",
      "Write-Host \"Wachten tot Windows de SD-kaart herkent...\" -ForegroundColor Gray",
      "Start-Sleep -Seconds 5",
      "",
      "# Stationsletter automatisch toewijzen via diskpart",
      "Write-Host \"Stationsletter automatisch toewijzen aan partition 1...\" -ForegroundColor Gray",
      "$assignedLetter = $null",
      "foreach ($tryLetter in @('E','F','G','H','I')) {",
      "    $volCheck = Get-Volume -DriveLetter $tryLetter -ErrorAction SilentlyContinue",
      "    if (-not $volCheck) {",
      "        $dpPost = \"select disk $diskNumber`r`nselect partition 1`r`nassign letter=$tryLetter`r`nexit\"",
      "        $dpPostTmp = \"$env:TEMP\\gridbox-diskpart-post.txt\"",
      "        [System.IO.File]::WriteAllText($dpPostTmp, $dpPost)",
      "        $dpPostOut = & diskpart /s $dpPostTmp 2>&1",
      "        Remove-Item $dpPostTmp -ErrorAction SilentlyContinue",
      "        $assignedLetter = $tryLetter",
      "        Write-Host \"Stationsletter $tryLetter toegewezen.\" -ForegroundColor Gray",
      "        break",
      "    }",
      "}",
      ...
      "    Write-Host \"\" ",
      "    Write-Host \"============================================\" -ForegroundColor Red",
      "    Write-Host \"  FOUT: Bootpartitie niet gevonden!\" -ForegroundColor Red",
      "    Write-Host \"============================================\" -ForegroundColor Red",
      "    Write-Host \"\"",
      ...
      "    Write-Host \"Kopieer box_bootstrap.json zelf naar de bootpartitie:\" -ForegroundColor Yellow",
      ...
      "",
      "    # cmdline.txt: alle ds=nocloud* tokens verwijderen en ds=nocloud;s=/boot/firmware/ toevoegen",
      ...
      "        $filtered = @($cmdlineParts | Where-Object { $_ -notmatch '^ds=nocloud' })",
      "        $filtered += 'ds=nocloud;s=/boot/firmware/'",
      "        [System.IO.File]::WriteAllText($CmdlinePath, ($filtered -join ' '), (New-Object System.Text.UTF8Encoding $false))",
      "        Write-Host \"cmdline.txt: ds=nocloud;s=/boot/firmware/ ingesteld\" -ForegroundColor Gray",
      "    } else {",
      "        Write-Host \"WAARSCHUWING: cmdline.txt niet gevonden op bootpartitie\" -ForegroundColor Yellow",
      "    }",
      "",
      "    # === Verificatie ===",
      "    Write-Host \"\"",
      ...
      "        if ($clText -match [regex]::Escape('ds=nocloud;s=/boot/firmware/')) {",
      "            Write-Host \"  $check cmdline.txt bevat ds=nocloud;s=/boot/firmware/\" -ForegroundColor Green",
      "        } else {",
      "            Write-Host \"  $cross cmdline.txt mist ds=nocloud;s=/boot/firmware/ — cloud-init vindt user-data NIET\" -ForegroundColor Red",
      "            $allOk = $false",
      "        }",
      "    } else {",
