  ...
  try {
    ...
    const cloudInitUserData = [
      ...
      ...(rpiConnectAuthKey ? [
        ...
        "      rpi-connect on",
        "      sleep 5",
        "      rpi-connect signin --auth-key \"$(cat /boot/firmware/rpi-connect-auth-key)\"",
        ...
        "      fi",
        "      rm -f /boot/firmware/rpi-connect-auth-key",
        ...
        "      Type=oneshot",
        "      User=pi",
        "      ExecStartPre=/bin/bash -c 'loginctl enable-linger pi && sleep 2'",
        "      ExecStart=/bin/bash -c 'XDG_RUNTIME_DIR=/run/user/1000 DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/1000/bus /usr/bin/bash /usr/local/bin/rpi-connect-setup.sh'",
        "      RemainAfterExit=yes",
        "      ",
        "      [Install]",
        "      WantedBy=multi-user.target"
