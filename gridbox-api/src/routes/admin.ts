  ...
  try {
    ...
    const cloudInitUserData = [
      ...
      ...(rpiConnectAuthKey ? [
        ...
        "      ExecStart=/bin/bash -c 'rpi-connect on && sleep 5 && rpi-connect signin --auth-key $(cat /boot/firmware/rpi-connect-auth-key) && systemctl --user enable rpi-connect && systemctl --user start rpi-connect && sleep 3 && RPI_ID=$(rpi-connect status 2>/dev/null | grep -oP \"(?<=device_id: )[\\w-]+\" || echo \"\") && BOX_ID=$(python3 -c \"import json; print(json.load(open(\\\"/boot/firmware/box_bootstrap.json\\\"))[\\\"boxId\\\"])\" 2>/dev/null || echo \"\") && API_URL=$(python3 -c \"import json; print(json.load(open(\\\"/boot/firmware/box_bootstrap.json\\\"))[\\\"apiBaseUrl\\\"])\" 2>/dev/null || echo \"\") && if [ -n \"$RPI_ID\" ] && [ -n \"$BOX_ID\" ]; then curl -s -X PATCH \"$API_URL/device/rpi-connect-register\" -H \"Content-Type: application/json\" -d \"{\\\"boxId\\\":\\\"$BOX_ID\\\",\\\"deviceId\\\":\\\"$RPI_ID\\\"}\"; fi && rm -f /boot/firmware/rpi-connect-auth-key'",
        "      RemainAfterExit=yes",
        "      ",
        "      [Install]",
        "      WantedBy=multi-user.target"
