...
auto_close_timer = None # De nieuwe bewaker
...
def get_snapshot_change_threshold():
    ...
    try:
        # We gebruiken nu de robuustere drempel van 25.0
        return float(cam_cfg.get("changeDetectionThreshold", 25.0))
    except Exception:
        return 25.0
...
def take_snapshot(phase="manual", sequence_number=None):
    ...
    try:
        ...
        # VERBOSE LOGGING IS TERUG:
        if not should_store:
            log(f"Snapshot overgeslagen | fase={phase} | seq={sequence_number} | score={change_score} | reden={store_reason}")
            return
        ...
        log(
            f"📸 Snapshot opgeslagen | fase={phase} | score={change_score} | reden={capture_reason} | {indexed_text}"
        )
...
# =========================================================
# COMMAND HANDLING + AUTO-CLOSE TIMER
# =========================================================
            ...
            if cmd == "OPEN":
                ...
                if ac_cfg.get("enabled", False):
                    ...
                    auto_close_timer = start_daemon_timer(
                        delay, 
                        lambda: handle_command(None, {"command": "CLOSE", "source": "Auto-Close System"})
                    )
                ...
                # Sloop de auto-close timer als we zelf sluiten
                cancel_timer(auto_close_timer)
...
try:
    ...
    cancel_timer(light_off_timer)
    cancel_timer(shutter_motor_timer)
    cancel_timer(auto_close_timer) # Alles opruimen bij afsluiten
    stop_shutter_motors()
    light.off()